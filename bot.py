...
def generate_content():
    try:
        # 1. Gemini se Text lena (Standard Model)
        print("Step 1: Fetching Script from Gemini...")
        
        # Model ka naam sirf 'gemini-1.5-flash' rakho
        response = client.models.generate_content(
            model="gemini-1.5-flash", 
            contents="Write a 1-sentence hyper-realistic English prompt for an AI to create a cinematic 9:16 portrait of Lord Krishna. Then on a new line, write a 10-word Hindi quote."
        )
        ...
        if len(img_data) > 5000:
            ...
            print("ERROR: Image download failed (size too small).")
            exit(1)
